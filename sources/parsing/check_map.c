...
/*   Updated: 2025/10/03 19:52:14 by pbret            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (i < 0 || i >= cub->map.nb_line || !map[i]
...
static bool	valid_outline(t_cub *cub, char **map, char c, int i, int j)
{
	bool	flag;
...
static bool	get_player(t_cub *cub, char c, int i, int j)
{
...
{printf("||||| CHECK_MAP |||||\n");
	...
	while (map[++i])
	{
		...
		while (map[i][++j])
		{
			...
			if (map[i][j] == '\n' && (j == 0 || i == cub->map.nb_line -1)) // pour gerer la derniere ligne vide de la map (si il y a) et une ligne vide en cours de map
