...
static t_error	check_rest_of_line(t_data *data, char **line)
{
	while(**line && **line != '\n')
	{
		if (**line != ' ')
...
static t_error	between_value(t_data *data, char **line, int nb_color, int *end)
{
	int	i;
	int comma;
	...
	while ((*line)[i] && ft_isdigit((*line)[i]))
		i++;
	*end = i; // stock la position de fin de serie des digits
	while ((*line)[i] && !ft_isdigit((*line)[i])) // gestion des char entre deux series de digits
	{	
		if (((*line)[i] != ',' && (*line)[i] != ' ' && (*line)[i] != '\n') // les 2 premieres sequences ->  char autorises [,][ ][\n]
			|| (nb_color > 1 && (*line)[i] != ' ' && (*line)[i] != '\n')) // [nb_color > 1] -> on est a la 3eme et derniere sequence; char autorise [ ]
			return (E_RGB_FT);
		else if ((*line)[i] == ',')
			comma++;
		i++;
	...
	printf("nb_color : %d\n", nb_color);
	if (comma != 1 && nb_color < 2) // il faut exatement une virgule entre les sequences; faut ignorer ce controle pour la derniere sequence
		return (E_RGB_FT);
	(*line)[*end] = '\0'; // remplacement du char apresla serie de digits par '\0'
...
static t_error	color_getter(t_data *data, char **line, t_key key_id)
{
	...
	while (i < 3)
	{
		err_id = between_value(data, line, i, &tmp_end);// retourne le nb de deplacement pour aller jusqu'a la prochaine digit
		if (err_id != OK)
			return (err_id);
...
static t_error	handle_line(t_data *data, char *line)
{
	...
	while (key_id <= C)
	{
		if (key_finder(&line, key_id))
		{
			...
			data->elem.e_counter++;
			key_id = handle_get_elem(data, line, key_id);
			if (key_id == OK)
				key_id = check_rest_of_line(data, &line);
			return (key_id);
		}
		key_id++;
	}
...
//		line = get_next_line(data->fd_file);
//		if (!line)
//			exit_door(data, "error malloc gnl");
